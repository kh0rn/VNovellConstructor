...
//+ИТ
//Установка нового элемента бэкгрунда
void UDialogueGraphSchema::GetBGAction(FGraphActionMenuBuilder& ActionMenuBuilder, const UEdGraph* CurrentGraph) const
{
	// Do not allow to spawn a comment when we drag are dragging from a selected pin.
	if (ActionMenuBuilder.FromPin)
	{
	...
	const bool bIsManyNodesSelected = CurrentGraph ? GetNodeSelectionCount(CurrentGraph) > 0 : false;
	const FText MenuDescription = bIsManyNodesSelected ?
		LOCTEXT("CreateBGAction", "Create back ground to join nodes") : LOCTEXT("AddBGAction", "Add Back ground for nodes...");
	const FText ToolTip = LOCTEXT("CreateBGToolTip", "Creates a back ground.");
	constexpr int32 Grouping = 1;
